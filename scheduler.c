...
void job_termination_handler(int signum);
...
void enqueue(pid_t pid) {
    ...
    if (!ready_tail) ready_head = ready_tail = node;
    else { ready_tail->next = node; ready_tail = node; }
}
...
// Handles SIGUSR1 signal for a new job submission
void new_job_handler(int signum) {
    pid_t new_pid;
    ...
    if (r == sizeof(new_pid)) {
        ...
        // Try to start the job immediately if a CPU slot is free
        for (int i = 0; i < ncpu; i++) {
            if (running_processes[i] == 0) {
                running_processes[i] = new_pid;
...
int main(int argc, char *argv[]) {
    ...
    raise(SIGALRM); // start immediately
